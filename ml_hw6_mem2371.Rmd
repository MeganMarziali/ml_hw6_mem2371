...
```{r}
set.seed(100)
svm.diabetes = svm(diabetes ~ ., data = train_svm, kernel = "linear", cost = 1, scale = TRUE)
print(svm.diabetes)

svm.pred = predict(svm.diabetes, newdata = train_svm[,1:10])
table(svm.pred, train_svm$diabetes)
confusionMatrix(svm.pred, train_svm$diabetes, positive = "Yes")

#Reminder this is how accuracy is calculated
misClasificError <- mean(svm.pred != training$diabetes, na.rm = T)
print(paste('Accuracy Model 1',1 - misClasificError))

features = train_svm[,1:10]
outcome = train_svm$diabetes
